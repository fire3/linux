...
int tsp_check_current(void)
{
	...
	while (vma) {
		...
		err = check_tsp_range(vma, start, end - start,
				      vma->vm_page_prot);
		if (err)
			break;
		vma = vma->vm_next;
...
int tsp_swap_current(void)
{
	...
	flush_tlb_all();
	current->mm->tsp->is_swapped = 1;
	up_write(&current->mm->mmap_sem);
	return err;
}

void free_tsp_page(struct page *page)
{
...
bool tsp_pud_huge_vma_suitable(struct vm_area_struct *vma, unsigned long haddr)
{
...
unlock_release:
#if 0
		printk("unlock_release do_tsp_huge_pmd_anonymous_page\n");
#endif
...
/*
 * Returns page table lock pointer if a given pmd maps a tsp, NULL otherwise.
 *
		     ...
		     pmd_t *pmd, unsigned long addr)
{
	...
	tsp_tlb_remove_pmd_tlb_entry(tlb, pmd, addr);
			...
			unsigned long address)
{
	...
	old_pmd = tsp_pmdp_invalidate(vma, haddr, pmd);
