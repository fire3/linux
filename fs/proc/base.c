			   ...
			   struct pid *pid, struct task_struct *task)
{
	...
	seq_printf(m, "code:  %#lx-%#lx %#lx-%#lx\n",
			...
			task->mm->smm_code_base_pfn << PAGE_SHIFT,
			(task->mm->smm_code_base_pfn << PAGE_SHIFT)
			+ task->mm->smm_code_page_count * PAGE_SIZE);

	seq_printf(m, "heap:  %#lx-%#lx %#lx-%#lx\n",
			task->mm->smm_heap_base_va,
			task->mm->smm_heap_end_va,
			task->mm->smm_mem_base_pfn << PAGE_SHIFT,
			(task->mm->smm_mem_base_pfn << PAGE_SHIFT)
			+ task->mm->smm_mem_page_count * PAGE_SIZE);
	...
	seq_printf(m, "stack: %#lx-%#lx %#lx-%#lx\n",
			...
			task->mm->smm_stack_base_pfn << PAGE_SHIFT,
			(task->mm->smm_stack_base_pfn << PAGE_SHIFT)
			+ task->mm->smm_stack_page_count * PAGE_SIZE);
	return 0;
}
#endif
